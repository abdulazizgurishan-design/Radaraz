...
export class PredictionEngine {
  ...
  static ruleBasedScore(fv, weights = {}) {
    ...
    // --------------------------
    // Structure
    // ✅ AUDIT FIX (phase 2): breakout / nearResistance are now REAL fields
    // computed in FeatureBuilder from daily bars (highest high over lookback).
    // Priority: confirmed breakout > near resistance > above VWAP > graded
    // VWAP distance. This restores the full 0.2–1.0 range for structure, so
    // genuinely strong setups can reach high scores.
    // --------------------------
    let structure;
    if (fv.breakout === true) {
      structure = 1.0;
