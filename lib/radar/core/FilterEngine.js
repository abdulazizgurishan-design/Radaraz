...
// ============================================================
// RadarAZ v20.1 - Filter Engine (مع تحسين RVOL)
// ============================================================
// ✅ إذا كان avgVolume غير موثوق (0 أو يساوي volume) → نتجاوز الفلتر
// ✅ إذا كان avgVolume حقيقياً → نطبق الفلتر
// ============================================================
...
export class FilterEngine {
  ...
  static applyRadarFilters(universe, options = {}) {
    ...
    const rvolFiltered = universe.filter(s => {
      ...
      // الحصول على avgVolume
      let avgVolume = Number(s.avgVolume ?? s.avg_volume ?? 0);

      // ✅ إذا كان avgVolume غير موثوق (0 أو يساوي volume الحالي) → نتجاوز الفلتر
      const isAvgVolumeReliable = avgVolume > 0 && avgVolume !== volume;

      if (!isAvgVolumeReliable) {
        // نسمح بالمرور (لا نستبعده)
        return true;
      }

      // ✅ إذا كان avgVolume موثوقاً → نطبق الفلتر الحقيقي
