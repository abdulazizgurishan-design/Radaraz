...
export class FilterEngine {
  ...
  static filter(universe, options = {}) {
    // ⚠️ TEMP CANARY TEST — REMOVE AFTER DIAGNOSIS.
    // Purpose: prove conclusively whether THIS exact filter() method is the
    // one executing in production, and whether scan.js's try/catch around
    // FilterEngine.filter() actually receives and logs the error.
    //
    // Expected in Vercel logs if this file/method IS what runs:
    //   "❌ [scan.js] FilterEngine.filter threw an error: FILTER ENGINE TEST"
    //
    // If that line does NOT appear after deploying and running /api/scan:
    //   → scan.js is not calling this filter() (wrong build, wrong import
    //     resolution, stale deployment, or a different FilterEngine instance
    //     entirely) — stop editing this file and locate the actual one.
    throw new Error('FILTER ENGINE TEST');
