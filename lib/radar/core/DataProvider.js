...
// ✅ مع سجلات DEBUG لفحص شكل البيانات القادمة من Polygon
// ============================================================
...
export class DataProvider {
  constructor() {
    ...
    // Previously a single CircuitBreaker was shared across ALL symbols, so
    // 3 unrelated failures (e.g. NVDA/SNDK/MU returning HTTP 400) would trip
    // the breaker OPEN and instantly fail every other symbol in the same
    // batch ("Circuit breaker is OPEN"), even though their requests were
    // never actually attempted. This is what caused most of a batch to fall
    // back to FeatureBuilder's NO_BARS fallback (ema9=ema21=price, rsi=50,
    // score=9.0) even though Polygon itself was healthy for those symbols.
  ...
  async getUniverse() {
    ...
    try {
      ...
      console.log('🔍 [DEBUG] lastQuote:', JSON.stringify(tickers[0]?.lastQuote, null, 2));
  ...
  async getBars(symbol, options = {}) {
    ...
    // ⚠️ TEMP DIAGNOSTIC: bars cache bypassed to get clean, uncontaminated
    // runs while debugging the "1 bar" issue. The in-memory cache was holding
    // results from older code versions (sort=asc / pre-pagination), making
    // every test run ambiguous. Set BARS_CACHE_DISABLED back to false once the
    // bar-count issue is resolved, to restore normal caching/performance.
    const BARS_CACHE_DISABLED = true;
    ...
    return this.rateLimiter.execute(async () => {
      try {
        ...
        if (timeframe === 'day') {
          ...
          // ✅ FIX: the old formula (limit * timeframe minutes) produced ~82
          // calendar days for hourly bars, but Polygon paginates such wide
          // ranges and a single page only returned the most recent day (~8
          // bars). Instead, size the window to how many trading days are
          // actually needed for `limit` intraday bars (~7 hourly bars/day),
          // capped to keep the whole result in one page. ceil(limit/7) days
          // of history, +3 buffer, capped at 20 trading-day-equivalents.
          ...
          // Convert trading days to calendar days (~1.4x for weekends).
        ...
        // ✅ FIX: parseInt('day') = NaN, which was sent to Polygon as an
        // invalid multiplier (/range/NaN/day/...), causing HTTP 400 on every
        // daily-bars request. Daily bars must use multiplier=1; intraday
        // timeframes ('60', '15', ...) are numeric and parse correctly.
        const multiplier = timeframe === 'day' ? 1 : parseInt(timeframe);
        const timespan = timeframe === 'day' ? 'day' : 'minute';

        // ✅ FIX (root cause of bars=8): Polygon returns bars in the order set
        // by `sort`, capped per page. With sort=asc the first page was the
        // OLDEST bars — often just the first day of the range (~8 hourly bars)
        // — with the rest behind next_url (pagination), which this code never
        // followed. Indicators need the MOST RECENT bars, so request sort=desc
        // (newest first, fills a full page with recent data in one request),
        // then reverse to chronological (oldest→newest) order for the
        // indicator math which expects ascending time.
        // ✅ FIX (true root cause of bars=8): Polygon caps each response page
        // and returns the remainder behind `next_url`. A single page of hourly
        // bars only covers ~1 trading day (~8 bars). With sort=desc the first
        // page is the most recent day; we then follow next_url to accumulate
        // older pages until we have enough bars (or hit a safety cap), then
        // reverse to chronological (oldest→newest) order for the indicators.
        // ✅ SIMPLER, ROBUST FIX (replaces fragile pagination that produced
        // "1 bar" results): request an amount of history that comfortably
        // fits in a single Polygon page and yields enough bars for the
        // indicators, using sort=desc so we always get the MOST RECENT bars.
        // For intraday we cap the effective request at 120 bars (well above
        // the ~26 needed for MACD/EMA21) — this fits one page and avoids the
        // multi-page cursor edge cases entirely. We then reverse to
        // chronological (oldest→newest) order for the indicator math.
        ...
        // TEMP DIAGNOSTIC — pure logging, no logic change.
        // Confirms the actual `limit` value received by this call and the
        // actual `results.length` returned by Polygon for that exact call,
        // so we can resolve the bars=8 vs bars=500 contradiction with real
  ...
  async getNews(symbol, limit = 1) {
    ...
    try {
      ...
      // ✅ FIX: was this.circuitBreaker (no longer exists after the per-symbol
      // Map refactor). Use the per-symbol breaker to avoid a crash if getNews
      // is ever called.
      const data = await this._getBreaker(symbol).execute(() =>
        fetchWithRetry(url, 3000, 2)
      );
