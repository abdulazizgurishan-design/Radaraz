...
// ============================================================
// RadarAZ v20.1 - Data Provider (مع تغيير minute → hour)
// ============================================================
...
export class DataProvider {
  ...
  async getUniverse() {
    ...
    try {
      ...
      if (tickers.length === 0) {
        console.error('❌ [getUniverse] No tickers returned.');
  ...
  async getBars(symbol, options = {}) {
    ...
    return this.rateLimiter.execute(async () => {
      try {
        ...
        // ✅ تغيير: للإطارات الدقيقة، استخدم hour بدلاً من minute
        let multiplier, timespan;
        if (timeframe === 'day') {
          multiplier = 1;
          timespan = 'day';
        } else {
          // ✅ استخدام hour بدلاً من minute
          multiplier = 1;
          timespan = 'hour';
        }
