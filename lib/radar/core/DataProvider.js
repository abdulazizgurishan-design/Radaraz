    ...
    return this.rateLimiter.execute(async () => {
      try {
        ...
        if (timeframe === 'day') {
          ...
          const minutesPerDay = 6.5 * 60;
          const minutesNeeded = limit * parseInt(timeframe);
          daysBack = Math.ceil(minutesNeeded / minutesPerDay) + 5;
        ...
        let rawResults = [];
        let pageUrl = `https://api.polygon.io/v2/aggs/ticker/${symbol}/range/${multiplier}/${timespan}/${fromStr}/${toStr}?adjusted=${adjusted}&sort=desc&limit=${limit}&apiKey=${POLYGON_KEY}`;
        let pageCount = 0;
        const MAX_PAGES = 10; // safety cap: up to ~10 pages of data

        while (pageUrl && pageCount < MAX_PAGES) {
          const pageData = await this._getBreaker(symbol).execute(() =>
            fetchWithRetry(pageUrl, 5000, 2)
          );
          const pageResults = pageData?.results || [];
          rawResults = rawResults.concat(pageResults);
          pageCount++;

          // Stop once we've collected enough bars for the requested limit.
          if (rawResults.length >= limit) break;

          // Polygon's next_url does not include the apiKey; append it.
          pageUrl = pageData?.next_url
            ? `${pageData.next_url}&apiKey=${POLYGON_KEY}`
            : null;
        }

        // Trim to the requested limit (newest `limit` bars), then reverse to
        // oldest-first chronological order for indicator math.
        const results = rawResults.slice(0, limit).reverse();
