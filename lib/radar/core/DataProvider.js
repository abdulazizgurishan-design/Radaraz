...
// ✅ Singleton | LRU Cache | Retry + Backoff | Circuit Breaker
// ✅ Adaptive TTL | RateLimiter Adaptive
// ✅ Daily Bars Cache منفصل (TTL أطول)
// ============================================================
...
class RateLimiter {
  ...
  async execute(fn) {
    ...
    // ✅ Adaptive Delay: إذا اقتربنا من 80% من الحد، نضيف تأخير
...
export class DataProvider {
  constructor() {
    ...
    this.dailyCache = new LRUCache(200); // ✅ Cache منفصل للـ Daily Bars
  ...
  _getTTL(type) {
    ...
    if (type === 'daily') return isOpen ? 300 : 3600; // ✅ Daily: 5 دقائق أثناء السوق
    ...
    try {
      ...
      const result = tickers.map(t => {
        ...
        return {
          ...
          avgVolume: d.v ?? 0, // ✅ تخزين متوسط الحجم للاستخدام
        };
  ...
  async getBars(symbol, options = {}) {
    ...
    // ✅ إذا كان Daily، استخدم Cache منفصل
