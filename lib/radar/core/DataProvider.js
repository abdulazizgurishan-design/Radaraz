...
// ✅ مع سجلات تشخيصية لـ getUniverse()
// ============================================================
...
export class DataProvider {
  ...
  // ─── getUniverse (مع سجلات تشخيصية) ──────────────────────
  async getUniverse() {
    const cacheKey = 'universe';
    const cached = this.cache.get(cacheKey);
    if (cached) {
      console.log(`✅ [getUniverse] Cache hit: ${cached.length} stocks`);
      return cached;
    }
    ...
    try {
      ...
      // ✅ سجلات تشخيصية
      console.log("🔍 [getUniverse] DATA KEYS:", Object.keys(data));
      console.log("🔍 [getUniverse] tickers =", data?.tickers?.length);
      console.log("🔍 [getUniverse] results =", data?.results?.length);
      console.log("🔍 [getUniverse] FULL RESPONSE (first 2000 chars):", JSON.stringify(data).substring(0, 2000));

      const tickers = data?.tickers || [];
      if (tickers.length === 0) {
        console.error('❌ [getUniverse] No tickers returned from Polygon. Check API key or endpoint.');
      ...
      console.log(`🔍 [getUniverse] After filter: ${result.length} stocks`);
    ...
    return this.rateLimiter.execute(async () => {
      try {
        ...
        if (timeframe === 'day') {
          ...
          const minutesNeeded = limit * parseInt(timeframe, 10);
        ...
        const multiplier = timeframe === 'day' ? 1 : parseInt(timeframe, 10);
