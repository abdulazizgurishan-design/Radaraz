...
export class FeatureBuilder {
  ...
  static _computeTargets(price, atr, resistance) {
    const atrStep = atr > 0 ? atr : price * 0.04; // خطوة احتياطية عند غياب ATR
    const atrTargets = atr > 0
      ? [
          price + atr * SCAN_CONFIG.TARGET1_MULTIPLIER,
          price + atr * SCAN_CONFIG.TARGET2_MULTIPLIER,
          price + atr * SCAN_CONFIG.TARGET3_MULTIPLIER,
        ]
      : [price * 1.04, price * 1.08, price * 1.12];

    const usedResistance = resistance != null && resistance > price;

    const candidates = [...atrTargets];
    if (usedResistance) candidates.push(resistance);

    // فوق السعر، تصاعدياً، مع إزالة المتقارب جداً (ضمن 0.1%).
    const merged = [];
    for (const c of candidates.filter(v => v > price).sort((a, b) => a - b)) {
      if (!merged.length || (c - merged[merged.length - 1]) / price > 0.001) merged.push(c);
    }
    // ضمان ثلاثة مستويات (مدّ بالخطوة إن نقصت بعد الدمج/الإزالة).
    while (merged.length < 3) {
      merged.push((merged[merged.length - 1] || price) + atrStep);
    }
    ...
    return {
      ...
      target2: merged[1],
      target3: merged[2],
      target_source: usedResistance
        ? 'blended_resistance_atr'
        : (atr > 0 ? 'atr_based' : 'percent_based'),
    };
  }
  ...
  static buildFromBars(stock, bars, marketContext, timeframe = '5', dailyBars = null) {
    ...
    let stop = atr > 0 ? price - atr * SCAN_CONFIG.STOP_MULTIPLIER : price * 0.97;
