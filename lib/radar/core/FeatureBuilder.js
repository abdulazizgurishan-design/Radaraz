...
export class FeatureBuilder {
  ...
  // ─────────────────────────────────────────────────────────
  // أهداف ممزوجة (v20.3): نُدرِج المقاومة داخل سلّم أهداف ATR ونأخذ أقرب
  // ثلاثة مستويات مميّزة فوق السعر. المقاومة عائق حقيقي فتصبح أحد الأهداف
  // حين تقع ضمن مدى ATR (فيتوقّف السعر عندها واقعياً)؛ وإن غابت (اختراق/قمم
  // جديدة) تبقى الأهداف ATR صرفة.
  // ─────────────────────────────────────────────────────────
  static _computeTargets(price, atr, resistance) {
    // فلسفة مختلطة (مبنية على البيانات): هدف أول قريب يؤمّن الربح،
    // وثالث بعيد يلاحق الذروة الفعلية (~+29% للرابحة). أهداف نسبية لا ATR
    // (ATR اليومي مضخّم فيبعّد الأهداف)، مع دمج المقاومة القريبة كهدف أول واقعي.
    const T1 = 0.05, T2 = 0.12, T3 = 0.22; // النسب المستهدفة
    const T1_MAX = 0.08;                    // سقف الهدف الأول (يمنع البعيد)

    let target1 = price * (1 + T1);
    let target2 = price * (1 + T2);
    let target3 = price * (1 + T3);
    let source = 'percent_mixed';

    // دمج بنيوي: مقاومة قريبة فوق السعر تصبح الهدف الأول الواقعي (السعر يتوقف عندها)
    if (resistance != null && resistance > price) {
      const resPct = (resistance - price) / price;
      if (resPct >= 0.015 && resPct <= T1_MAX) {
        target1 = resistance;
        source = 'blended_resistance';
      }
    }

    // ضمان الترتيب التصاعدي
    if (target2 <= target1) target2 = target1 * 1.04;
    if (target3 <= target2) target3 = target2 * 1.04;

    return { target1, target2, target3, target_source: source };
  }

  // ─────────────────────────────────────────────────────────
  // وقف متكيّف بسقف نسبي (مبني على البيانات: الرابحة تتراجع ~2.7% وسيطاً
  // قبل انطلاقها، فالوقف يجب أن يكون أوسع من ذلك بهامش، لكن أضيق بكثير من
  // الوقف اليومي المكسور -8%). النطاق: [2.5%, 4.5%] من السعر.
  //   - يفضّل الوقف تحت الدعم القريب إن كان ضمن النطاق.
  //   - يقصّ أي وقف أوسع من 4.5% (يمنع النزيف العميق).
  // ─────────────────────────────────────────────────────────
  static _computeStop(price, atr, support) {
    const STOP_MIN = 0.025; // أضيق حدّ (يتجنّب الضجيج)
    const STOP_MAX = 0.045; // أوسع حدّ (يمنع الوقف الواسع)

    let distPct = 0.04; // افتراضي
    if (atr > 0 && price > 0) {
      distPct = (atr * SCAN_CONFIG.STOP_MULTIPLIER) / price;
    }
    // دعم قريب: ضع الوقف تحته بقليل إن كان أضيق من حساب ATR
    if (support != null && support < price && support > 0) {
      const supDist = (price - support) / price + 0.005;
      distPct = Math.min(distPct, supDist);
    }
    distPct = Math.max(STOP_MIN, Math.min(STOP_MAX, distPct));
    return price * (1 - distPct);
  }
  ...
  static buildFromBars(stock, bars, marketContext, timeframe = '5', dailyBars = null) {
    ...
    // ─── الوقف (ATR/نسبة) + أهداف ممزوجة بالمقاومة (v20.3) ──
    // ─── الوقف المتكيّف بسقف نسبي + أهداف مختلطة (v20.4) ──
    let stop = this._computeStop(price, atr, structure.support);
    const _tg = this._computeTargets(price, atr, structure.resistance);
    let target1 = _tg.target1;
    let target2 = _tg.target2;
    let target3 = _tg.target3;
    const target_source = _tg.target_source;
  ...
  static _buildFallback(stock, marketContext, timeframe = '5', reason = 'UNKNOWN') {
    ...
    // v20.4: وقف متكيّف + أهداف مختلطة (نفس منطق المسار الرئيسي)
    const stop = this._computeStop(price, atr, null);
    const _tgf = this._computeTargets(price, atr, null);
    const target1 = _tgf.target1, target2 = _tgf.target2, target3 = _tgf.target3;
    const target_source = 'fallback_' + _tgf.target_source;
  ...
  static build(stock, marketContext) {
    ...
    // v20.4: وقف متكيّف + أهداف مختلطة
    const stop = this._computeStop(price, atr, null);
    const _tgb = this._computeTargets(price, atr, null);
    const target1 = _tgb.target1, target2 = _tgb.target2, target3 = _tgb.target3;
    const target_source = 'legacy_' + _tgb.target_source;
