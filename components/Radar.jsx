...
export default function Radar() {
  ...
  const handleScan = async () => {
    setLoading(true);
    try {
      // إضافة رمز عشوائي للرابط لمنع المتصفح والـ Vercel من قراءة أي كاش قديم قسراً
      const response = await fetch(`/api/scan?nocache=${Date.now()}`);
      const result = await response.json();
      
      if (result.success && result.data && result.data.length > 0) {
        setStocks(result.data);
        
        // حساب الإحصائيات من البيانات القادمة فوراً
        const explosionsCount = result.data.filter(s => s.volume > 300000).length;
        setStats({
          total: 8000,
        ...
        alert("تنبيه: السيرفر لم يرسل بيانات، تحقق من اتصال الشبكة.");
      }
    } catch (error) {
  ...
  return (
    <div style={{ backgroundColor: '#0f172a', color: '#fff', minHeight: '100vh', padding: '20px', fontFamily: 'sans-serif', direction: 'rtl' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '30px' }}>
        ...
        <p style={{ color: '#94a3b8' }}>منصة متطورة لمسح وفلترة ميكرو كاب السوق الأمريكي بالكامل لحظياً للشركات النقية مالياً</p>
      </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: '30px' }}>
        ...
        >
          {loading ? "⏳ جاري تشغيل الرادار وسحب الصفقات..." : "📡 ابدأ مسح السوق الفوري"}
        </button>
      </div>
            ...
            <table style={{ width: '100%', textAlign: 'right', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #334155', color: '#94a3b8' }}>
                  ...
                  <th style={{ padding: '10px' }}>نسبة الديون</th>
                  <th style={{ padding: '10px' }}>الوضع الفني</th>
                </tr>
              </thead>
              <tbody>
                {stocks.map((stock, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #334155', height: '50px' }}>
                    ...
                    <td style={{ padding: '10px', color: '#10b981' }}>{stock.debtRatio}</td>
                    <td style={{ padding: '10px', color: stock.volume > 300000 ? '#ef4444' : '#10b981' }}>{stock.signal}</td>
                  </tr>
                ))}
              </tbody>
            </table>
