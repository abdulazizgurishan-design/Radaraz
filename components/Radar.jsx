...
function ScoreBar({score}){
  ...
  return(
    <div style={{position:"relative",height:3,background:"rgba(255,255,255,0.08)",borderRadius:3,overflow:"hidden",marginTop:6}}>
      <div style={{position:"absolute",left:0,top:0,height:"100%",width:`${score}%`,background:color,borderRadius:3,transition:"width 1s ease",boxShadow:`0 0 8px ${color}`}}/>
    </div>
  );
}
...
function Card({r,idx}){
  ...
  return(
    <div style={{background:"linear-gradient(135deg,rgba(15,20,35,0.95),rgba(20,28,48,0.95))",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,marginBottom:10,overflow:"hidden",transition:"all 0.3s",boxShadow:open?`0 8px 32px ${glowColor},0 0 0 1px rgba(255,255,255,0.05)`:"0 2px 8px rgba(0,0,0,0.3)"}}>
      <div onClick={()=>setOpen(o=>!o)} style={{padding:"16px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        <span style={{fontSize:10,color:"rgba(255,255,255,0.2)",minWidth:22,fontFamily:"'Courier New',monospace"}}>{String(idx+1).padStart(2,"0")}</span>
        <div style={{minWidth:64}}>
          <div style={{fontSize:17,fontWeight:700,color:"#fff",letterSpacing:1,fontFamily:"'Courier New',monospace"}}>{r.symbol}</div>
        ...
        <div style={{textAlign:"right",minWidth:80}}>
          <div style={{fontSize:18,fontWeight:700,color:"#fff",fontFamily:"'Courier New',monospace"}}>{f$(r.price)}</div>
          <div style={{fontSize:12,color:r.change_pct>=0?"#00d4aa":"#ff4757",fontWeight:600}}>{fp(r.change_pct)}</div>
        </div>
        <div style={{textAlign:"center",minWidth:44}}>
          <div style={{fontSize:20,fontWeight:800,color:scoreColor,fontFamily:"'Courier New',monospace",lineHeight:1}}>{r.score}</div>
      ...
      {open&&(
        <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"16px 18px"}}>
          <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            {[{l:"EMA 9",v:r.ema9?f$(r.ema9):"—",c:"#a78bfa"},{l:"EMA 20",v:r.ema20?f$(r.ema20):"—",c:"#fbbf24"},{l:"VWAP",v:r.vwap?f$(r.vwap):"—",c:"#60a5fa"},{l:"RVOL",v:r.rvol.toFixed(1)+"x",c:"#fb923c"},{l:"حجم",v:(r.volume/1e6).toFixed(1)+"M",c:"#34d399"}].map(i=>(
              <div key={i.l} style={{flex:1,minWidth:60,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"8px 10px"}}>
                ...
                <div style={{fontSize:13,color:i.c,fontWeight:700,fontFamily:"'Courier New',monospace"}}>{i.v}</div>
              </div>
            ))}
          </div>
          <div style={{background:"linear-gradient(135deg,rgba(255,71,87,0.1),rgba(255,71,87,0.05))",border:"1px solid rgba(255,71,87,0.2)",borderRadius:12,padding:"14px 16px",marginBottom:10}}>
            <div style={{fontSize:10,color:"#ff6b81",fontWeight:600,marginBottom:8}}>🛑 وقف الخسارة — Low of Day</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:22,fontWeight:700,color:"#ff4757",fontFamily:"'Courier New',monospace"}}>{f$(r.levels.sl)}</span>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {[{n:1,v:r.levels.t1,p:15,l:"TP1",c:"#60a5fa",bg:"rgba(96,165,250,0.08)",border:"rgba(96,165,250,0.2)"},{n:2,v:r.levels.t2,p:30,l:"TP2",c:"#34d399",bg:"rgba(52,211,153,0.08)",border:"rgba(52,211,153,0.2)"},{n:3,v:r.levels.t3,p:50,l:"TP3",c:"#fbbf24",bg:"rgba(251,191,36,0.08)",border:"rgba(251,191,36,0.2)"}].map(t=>(
              <div key={t.n} style={{background:t.bg,border:`1px solid ${t.border}`,borderRadius:12,padding:"12px"}}>
                ...
                <div style={{fontSize:16,fontWeight:700,color:t.c,fontFamily:"'Courier New',monospace"}}>{f$(t.v)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:920,margin:"0 auto",padding:"24px 16px"}}>
        <div style={{textAlign:"center",marginBottom:32,paddingBottom:24,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12,marginBottom:10}}>
            <div style={{width:10,height:10,borderRadius:"50%",background:loading?"#ffd700":done&&results.length>0?"#00d4aa":"#ff6b35",boxShadow:`0 0 16px ${loading?"#ffd700":done&&results.length>0?"#00d4aa":"#ff6b35"}`,animation:loading?"blink 0.6s infinite":"breathe 3s ease-in-out infinite"}}/>
          ...
          <p style={{margin:0,fontSize:12,color:"rgba(255,255,255,0.35)",letterSpacing:1}}>أسهم أمريكية شرعية · ماركت كاب &lt; $200M · تحليل لحظي</p>
        </div>
        <div style={{display:"flex",gap:10,marginBottom:24,flexWrap:"wrap"}}>
          {[{l:"نطاق الفحص",v:total||100,c:"#6366f1",bg:"rgba(99,102,241,0.1)",border:"rgba(99,102,241,0.2)"},{l:"💥 انفجاري",v:explosive,c:"#ff6b35",bg:"rgba(255,107,53,0.1)",border:"rgba(255,107,53,0.2)"},{l:"🔥 عالي",v:high,c:"#ffd700",bg:"rgba(255,215,0,0.1)",border:"rgba(255,215,0,0.2)"},{l:"✅ الكل",v:results.length,c:"#00d4aa",bg:"rgba(0,212,170,0.1)",border:"rgba(0,212,170,0.2)"}].map(s=>(
            <div key={s.l} style={{flex:1,minWidth:80,background:s.bg,border:`1px solid ${s.border}`,borderRadius:14,padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:26,fontWeight:900,color:s.c,fontFamily:"'Courier New',monospace",lineHeight:1}}>{s.v}</div>
              <div style={{fontSize:9,color:s.c,opacity:0.7,marginTop:4,letterSpacing:0.5}}>{s.l}</div>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
          <button onClick={scan} disabled={loading} style={{flex:1,background:loading?"rgba(255,255,255,0.05)":"linear-gradient(135deg,#6366f1,#8b5cf6)",border:loading?"1px solid rgba(255,255,255,0.1)":"none",borderRadius:14,padding:"14px 28px",color:loading?"rgba(255,255,255,0.3)":"#fff",fontWeight:800,fontSize:14,cursor:loading?"not-allowed":"pointer",letterSpacing:1,boxShadow:loading?"none":"0 8px 32px rgba(99,102,241,0.4)",transition:"all 0.3s"}}>
        ...
        {loading&&<div style={{height:2,background:"rgba(255,255,255,0.05)",borderRadius:2,marginBottom:20,overflow:"hidden"}}><div style={{height:"100%",width:"65%",background:"linear-gradient(90deg,#6366f1,#8b5cf6)",borderRadius:2}}/></div>}
        {filtered.length>0&&(
          <>
        ...
        <div style={{marginTop:32,paddingTop:16,borderTop:"1px solid rgba(255,255,255,0.04)",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.2)",letterSpacing:2,fontFamily:"'Courier New',monospace"}}>RADAR AZ PRO</span>
          <span style={{fontSize:10,color:"rgba(255,255,255,0.15)",fontStyle:"italic"}}>أسهم شرعية · ليست نصيحة استثمارية</span>
        </div>
      </div>
      <style>{`@keyframes blink{0%,100%{opacity:1}50%{opacity:0.2}}@keyframes breathe{0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.6;transform:scale(0.8)}}*{box-sizing:border-box;}::-webkit-scrollbar{width:4px;}::-webkit-scrollbar-track{background:#080c18;}::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:4px;}button:hover:not(:disabled){filter:brightness(1.1);}`}</style>
    </div>
  );
}
