// pages/api/scan.js — STANDALONE EDITION v4 (OPTIMIZED)
// ═══════════════════════════════════════════════════════════════════
//  تحسينات الإصدار:
//   ✅ HEAVY_LIMIT: 45 → 60 (تحليل أكثر)
//   ✅ DEADLINE: 6.5s → 20s (استغلال وقت Vercel)
//   ✅ STRETCH أوسع: WARN 10→12, PEN_K 0.4→0.2, DROP 18→25
//   ✅ أخبار شرطية: فقط للأسهم الواعدة (يقلل الطلبات)
//   ✅ MIN_RR 1.5→1.3, MAX_POS 0.66→0.75
// ═══════════════════════════════════════════════════════════════════
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await inBatches(top, 30, async (s) => {
      ...
      const wantNews = s.changePct > 15 || s.rvol > 10 || s.ep >= 75;
