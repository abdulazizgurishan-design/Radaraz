...
function MarketMovers({ movers, signals, t, lang }) {
  ...
  // ── جلب البنية الكاملة عند الضغط ──
  // الأولوية: (1) إشارة اليوم إن كانت محلّلة → فوري بلا طلب. وإلا
  // (2) /api/structure لجلب بنية كاملة لأي سهم. وإلا (3) بيانات أساسية.
  ...
  const toggleStruct = async (sym) => {
    ...
    // موجود في إشارات اليوم؟ اعرض بنيته فوراً بلا طلب شبكة.
    const local = findSignal(sym);
    if (local && local.structure) {
      setStructData((p) => ({ ...p, [sym]: { loading: false, price: local.price, structure: local.structure, levels: local.levels } }));
      return;
    }
    // مخزّن مسبقاً؟
    ...
    try {
      ...
      const data = await res.json();
      setStructData((p) => ({ ...p, [sym]: { loading: false, price: data.price, structure: data.structure || null } }));
    } catch {
                ...
                <div style={{ padding: "4px 14px 14px", background: "rgba(6,10,20,0.5)" }}>
                  ...
                  {sd && !sd.loading && sd.structure && (
                    <div style={{ background: "rgba(6,10,20,0.6)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 12 }}>
                      ...
                      <StructureMap r={{ price: sd.price || m.price, structure: sd.structure, levels: sd.levels }} lang={lang} />
                    </div>
                  )}
                  {sd && !sd.loading && !sd.structure && (
                    <div style={{ padding: "12px 14px", background: "rgba(6,10,20,0.6)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, fontSize: 12.5, color: "rgba(255,255,255,0.7)", lineHeight: 1.8 }}>
                      ...
                      <div style={{ marginTop: 8, fontSize: 11, color: "rgba(255,255,255,0.35)" }}>
                        ℹ️ {en ? "No structure data available for this stock." : "لا تتوفر بيانات بنية كافية لهذا السهم."}
                      </div>
                    </div>
                  )}
                </div>
