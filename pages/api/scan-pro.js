...
// يرجع كائن { maBonus, maSignal } — يرفع EP فقط، لا يستبعد
async function fetchMACrossover(symbol) {
  try {
    // آخر 30 يوم تداول (يكفي لـ MA21 + EMA)
    ...
    if (!res.ok) return { maBonus: 0, maSignal: null };
    const data = await res.json();
    const bars = data.results || [];
    if (bars.length < 21) return { maBonus: 0, maSignal: null };
    ...
    return { maBonus: Math.min(maBonus, 20), maSignal };
  } catch {
    return { maBonus: 0, maSignal: null };
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await Promise.all(top.map(async (s) => {
      const { maBonus, maSignal } = await fetchMACrossover(s.symbol);
      if (maBonus > 0) {
