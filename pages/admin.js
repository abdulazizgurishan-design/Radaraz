...
export default function Admin() {
  ...
  const copyTweet = () => {
    navigator.clipboard.writeText(buildTweet());
    setCopied(true);
  ...
  const S = {
    ...
    tag: (bg, color) => ({ fontSize: 11, padding: "3px 8px", borderRadius: 20, background: bg, color }),
        ...
        {summary && (
          <>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
              {[
                { label: "إشارات", value: summary.total, color: "#6366f1" },
                { label: "✅ T1", value: summary.t1, color: "#00d4aa" },
                { label: "🎯 T2", value: summary.t2, color: "#60a5fa" },
                { label: "❌ وقف", value: summary.stops, color: "#ff4757" },
              ].map(s => (
                <div key={s.label} style={{ flex: 1, minWidth: 80, background: "rgba(255,255,255,0.04)", border: `1px solid rgba(255,255,255,0.08)`, borderRadius: 12, padding: "12px", textAlign: "center" }}>
              ...
              return (
                <div key={s.id} style={S.card(borderColor)}>
                  ...
                  <div style={{ textAlign: "left" }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: s.close_gain_pct >= 0 ? "#00d4aa" : "#ff4757", fontFamily: "monospace" }}>
                      {s.close_gain_pct >= 0 ? "+" : ""}{s.close_gain_pct}%
                    </div>
